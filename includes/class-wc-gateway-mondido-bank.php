...
class WC_Gateway_Mondido_Bank extends WC_Gateway_Mondido_HW {
	/**
	 * Init
	 */
	public function __construct() {
		...
		$this->icon     = apply_filters( 'woocommerce_mondido_bank_icon', plugins_url( '/assets/images/bank.png', dirname( __FILE__ ) ) );
		$this->supports = array(
			'products',
			'refunds',
		);
		...
		$this->title       = isset( $this->settings['title'] ) ? $this->settings['title'] : __( 'Mondido Direct Bank', 'woocommerce-gateway-mondido' );
		...
		$this->order_button_text = isset( $this->settings['order_button_text'] ) ? $this->settings['order_button_text'] : __( 'Pay with Direct Bank', 'woocommerce-gateway-mondido' );
	...
	public function init_form_fields() {
		$this->form_fields = array(
			'enabled'           => array(
				...
				'type'    => 'checkbox',
				'label'   => __( 'Enable Mondido Direct Bank', 'woocommerce-gateway-mondido' ),
				'default' => 'no'
			),
			'title'             => array(
				...
				'default'     => __( 'Mondido Direct Bank', 'woocommerce-gateway-mondido' )
			),
			'description'       => array(
			...
			'order_button_text' => array(
				...
				'type'    => 'text',
				'default' => __( 'Pay with Direct Bank', 'woocommerce-gateway-mondido' ),
			),
		);
	}
